...
internal sealed class DuplicateCandidate
{
    public string Level { get; init; } = string.Empty;
    public int Score { get; init; }
    public string Reason { get; init; } = string.Empty;
    public DuplicateOperation Left { get; init; } = null!;
    public DuplicateOperation Right { get; init; } = null!;
}
...
internal static class DuplicateAnalysisService
{
    public static List<DuplicateCandidate> Analyze()
    {
        ...
        // Banque et compte sont volontairement absents des critères de comparaison.
        var groups = operations.GroupBy(x => new { Amount = decimal.Round(x.Amount, 2), Currency = Normalize(x.Currency) });
        foreach (var group in groups)
        {
            ...
            for (var i = 0; i < items.Count; i++)
            {
                ...
                {
                    var days = Math.Abs((items[j].Date.Date - items[i].Date.Date).Days);
                    if (days > 2) break;

                    var leftText = Normalize($"{items[i].Nature} {items[i].Label} {items[i].Details}");
                    var rightText = Normalize($"{items[j].Nature} {items[j].Label} {items[j].Details}");
                    var similarity = Similarity(leftText, rightText);
                    var sameDay = days == 0;
                    ...
                    var score = 45; // même montant + même devise
                    if (sameDay) score += 30;
                    else if (days == 1) score += 18;
                    else score += 10;
                    score += (int)Math.Round(similarity * 25m);
                    if (sameValueDate) score += 5;
                    score = Math.Min(score, 100);

                    if (score < 70) continue;
                    var level = score >= 90 ? "Certain" : "Probable";
                    var reason = $"Même montant {items[i].Amount:N2} {items[i].Currency}; dates à {days} jour(s); libellés similaires à {similarity:P0}";
                    results.Add(new DuplicateCandidate { Level = level, Score = score, Reason = reason, Left = items[i], Right = items[j] });
                }
            }
        }
    ...
    {
        if (left == right && left.Length > 0) return 1m;
...
internal sealed class DuplicateAnalysisForm : Form
{
    ...
    {
        Text = "QNB - Analyse globale des doublons";
        ...
        header.Controls.Add(new Label { Text = "Comparaison indépendante de la banque et du compte • montant, date, devise et similarité des libellés", AutoSize = true, ForeColor = Color.FromArgb(183, 207, 229), Location = new Point(20, 44) });
        _summary = new Label { AutoSize = true, Anchor = AnchorStyles.Top | AnchorStyles.Right, ForeColor = Color.FromArgb(58, 196, 187), Font = new Font("Segoe UI Semibold", 10F, FontStyle.Bold), Location = new Point(1080, 28) };
        ...
        _grid = new DataGridView
        {
            ...
            BackgroundColor = Color.FromArgb(7, 42, 78), ForeColor = Color.FromArgb(20, 30, 45),
        ...
        refresh.Click += (_, _) => LoadCandidates();
        footer.Controls.Add(close); footer.Controls.Add(refresh);

        Controls.Add(_grid); Controls.Add(footer); Controls.Add(header);
        Shown += (_, _) => LoadCandidates();
    }

    private void AddColumns()
    {
        Add("Niveau", 75); Add("Score", 55); Add("Date A", 85); Add("Montant A", 95); Add("Banque A", 120); Add("Compte A", 130); Add("Libellé A", 220);
        Add("Date B", 85); Add("Montant B", 95); Add("Banque B", 120); Add("Compte B", 130); Add("Libellé B", 220); Add("Motif", 330);
    }
    ...
    private void LoadCandidates()
    {
        ...
        try
        {
            _candidates = DuplicateAnalysisService.Analyze();
            _grid.Rows.Clear();
            var culture = CultureInfo.GetCultureInfo("fr-FR");
            foreach (var c in _candidates)
            {
                _grid.Rows.Add(c.Level, c.Score + "%", c.Left.Date.ToString("dd/MM/yyyy"), c.Left.Amount.ToString("N2", culture), c.Left.Bank, c.Left.Account, BestLabel(c.Left),
                    c.Right.Date.ToString("dd/MM/yyyy"), c.Right.Amount.ToString("N2", culture), c.Right.Bank, c.Right.Account, BestLabel(c.Right), c.Reason);
            }
