...
public sealed class AppUsageInfo
{
    ...
    public static AppUsageInfo LoadAndRegisterCurrentUse()
    {
        ...
        if (!File.Exists(ResetMarkerPath))
        {
            ...
            try { File.WriteAllText(DataFilePath, JsonSerializer.Serialize(fresh, new JsonSerializerOptions { WriteIndented = true })); File.WriteAllText(ResetMarkerPath, now.ToString("O")); } catch { }
            return fresh;
        }
        AppUsageInfo info;
        try
        {
            if (File.Exists(DataFilePath)) { var json = File.ReadAllText(DataFilePath); info = JsonSerializer.Deserialize<AppUsageInfo>(json) ?? new AppUsageInfo(); if (info.CreationDate == default) info.CreationDate = now; info.PreviousUseDate = info.LastUseDate == default ? null : info.LastUseDate; info.LastUseDate = now; }
            else info = new AppUsageInfo { CreationDate = now, PreviousUseDate = null, LastUseDate = now };
            File.WriteAllText(DataFilePath, JsonSerializer.Serialize(info, new JsonSerializerOptions { WriteIndented = true }));
        }
        catch { info = new AppUsageInfo { CreationDate = now, PreviousUseDate = null, LastUseDate = now }; }
        return info;
    }
}
