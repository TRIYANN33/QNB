...
internal sealed class BankAccountProfile { public Guid Id { get; set; } = Guid.NewGuid(); public string BankName { get; set; } = string.Empty; public string AccountName { get; set; } = string.Empty; public string AccountReference { get; set; } = string.Empty; public string Holder { get; set; } = string.Empty; public BankAccountType Type { get; set; } = BankAccountType.Courant; public string DisplayName => $"{BankName} — {AccountName}"; }
...
internal static class BankingRepository
{
    ...
    {
        Directory.CreateDirectory(Root); using var connection = OpenConnection(); using var command = connection.CreateCommand();
...
CREATE INDEX IF NOT EXISTS IX_Operations_ImportId ON Operations(ImportId); CREATE INDEX IF NOT EXISTS IX_Imports_AccountId ON Imports(AccountId);"; command.ExecuteNonQuery();
    ...
    {
        var cfg = new BankingConfiguration(); using var c = OpenConnection(); using var cmd = c.CreateCommand(); cmd.CommandText = "SELECT Id,BankName,AccountName,AccountReference,Holder,Type FROM Accounts ORDER BY BankName,AccountName"; using var r = cmd.ExecuteReader();
        while (r.Read()) cfg.Accounts.Add(new BankAccountProfile { Id=Guid.Parse(r.GetString(0)), BankName=r.GetString(1), AccountName=r.GetString(2), AccountReference=r.GetString(3), Holder=r.GetString(4), Type=(BankAccountType)r.GetInt32(5) }); return cfg;
    }
